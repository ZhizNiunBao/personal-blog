---
title: 数仓-07-DIM层
typora-copy-images-to: ./数仓-07-DIM层
typora-root-url: ./数仓-07-DIM层
...
select
	sku_id,
	collect_set(named_struct('attr_id',
	attr_id,
	'value_id',
	value_id,
	'attr_name',
...
select
	sku_id,
	collect_set(named_struct('sale_attr_id',
	sale_attr_id,
	'sale_attr_value_id',
	sale_attr_value_id,
	'sale_attr_name',
...
insert
	overwrite table dim_sku_full partition(dt = '2023-05-22')
select
	sku.id,
	sku.price,
	sku.sku_name,
...
left join attr on
	sku.id = attr.sku_id
left join sale_attr on
	sku.id = sale_attr.sku_id;


insert overwrite table dim_coupon_full partition(dt='2023-05-22')
select
	id,
	coupon_name,
	coupon_type,
...
on
	ci.range_type = range_dic.dic_code;
...
insert overwrite table dim_activity_full partition(dt='2023-05-22')
select	
	rule.id as rule_id,
	info.id as info_id,
	activity_name,
...
on
	rule.activity_type = dic.dic_code;

insert overwrite table dim_province_full partition(dt = '2023-05-22')
select
	province.id,
	province.name,
	province.area_code,
 ...
 ( 
	...
	from
		...
		select
			...
			data.create_time,
			data.operate_time,
			row_number() over (partition by data.id
		order by
			ts desc) rn
		from
			ods_user_info_inc
...
select
	...
	old_operate_time,
	old_start_date,
	cast(date_add('2023-05-22',-1) as string) old_end_date,
	cast(date_add('2023-05-22',-1) as string) dt     
from
	tmp
where
	old_id is not null
